...
original.yaml:
	curl https://docs.api.epilot.io/email-template.yaml?v > original.yaml
...
overlay.yaml: original.yaml original_modified.yaml
	speakeasy overlay compare -s original.yaml -s original_modified.yaml > overlay.yaml

speakeasy:
	$(eval TMP := $(shell mktemp -d))
	curl https://docs.api.epilot.io/email-template.yaml?v > $(TMP)/openapi.yaml
